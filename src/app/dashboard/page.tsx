            ...
            <div className="bg-white rounded-2xl border border-brand-100 p-6">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.label} className="flex justify-between items-center py-2.5 border-b border-brand-50">
                ...
                <div className="flex justify-between items-center pt-3 mt-1 border-t-2 border-brand-100">
                  <span className="font-heading font-semibold text-brand-900">Итого</span>
                  <span className="text-xl font-bold text-brand-500">
                    {formatMoney(Number(salary.total))}
                  </span>
                </div>
                <div className="flex justify-between items-center bg-accent-light rounded-xl px-4 py-3 -mx-1">
                  <span className="text-sm font-medium text-accent-dark">Прогноз</span>
          ...
          <div className="bg-white rounded-2xl border border-brand-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="text-sm text-brand-400 hover:text-brand-600 font-medium flex items-center gap-1 transition"
              >
            ...
            <div className="space-y-2">
              {d.recent_deals.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-8">Нет сделок за этот период</p>
              ) : (
                d.recent_deals.map((deal: any, i: number) => (
                  <div key={deal.id || i} className="flex items-center justify-between py-3 px-4 rounded-xl hover:bg-brand-50 transition">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-brand-50 flex items-center justify-center text-brand-500 font-bold text-sm">
                        {deal.client_name?.charAt(0) || '?'}
                      </div>
                      <div>
                        <p className="text-sm font-medium text-brand-900">{deal.client_name}</p>
                        <p className="text-xs text-gray-400">{deal.units} точек</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className={cn(
                      ...
                      <span className="text-sm font-semibold text-brand-900 w-28 text-right">
